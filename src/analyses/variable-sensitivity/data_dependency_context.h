...
private:
  ...
  // Templated so that it works on both ordered and un-ordered sets of locations
 ...
 */
template <class AOT>
class data_dependency_context_instancet:
  public data_dependency_contextt
{
public:
  explicit data_dependency_context_instancet(const typet &type):
    data_dependency_contextt(
      abstract_object_pointert(new AOT(type)), type) {}

  data_dependency_context_instancet(
    const typet &type,
    bool top,
    bool bottom):
      data_dependency_contextt(
  ...
  explicit data_dependency_context_instancet(
    const exprt &expr,
    const abstract_environmentt &environment,
    const namespacet &ns):
      data_dependency_contextt(
